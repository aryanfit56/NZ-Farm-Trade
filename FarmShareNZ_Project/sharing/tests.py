...
class FarmShareTests(TestCase):
 ...
 def test_signup_creates_farm_user_group(self):
  ...
  self.assertTrue(User.objects.get(username='newfarmer').groups.filter(name='Farm Users').exists())

 def test_login_required_for_create(self):
  self.assertEqual(self.client.get(reverse('equipment_create')).status_code,302)

 def test_non_owner_cannot_edit(self):
  self.client.login(username='other',password='StrongPass123!')
  self.assertEqual(self.client.get(reverse('equipment_update',args=[self.eq.pk])).status_code,403)

 def test_owner_can_edit(self):
  self.client.login(username='owner',password='StrongPass123!')
  self.assertEqual(self.client.get(reverse('equipment_update',args=[self.eq.pk])).status_code,200)

 def test_cannot_request_own_equipment(self):
  self.client.login(username='owner',password='StrongPass123!')
  self.assertEqual(self.client.get(reverse('request_create',args=[self.eq.pk])).status_code,403)

 def test_invalid_dates_rejected(self):
  self.client.login(username='other',password='StrongPass123!')
  today=timezone.localdate()
  r=self.client.post(reverse('request_create',args=[self.eq.pk]),{'start_date':today+timedelta(days=2),'end_date':today+timedelta(days=1),'message':'Need it'})
  self.assertEqual(RentalRequest.objects.count(),0)
  self.assertContains(r,'End date must be on or after start date.')
 ...
 def test_only_owner_can_approve(self):
  ...
  self.client.login(username='other',password='StrongPass123!')
  self.assertEqual(self.client.post(reverse('request_status',args=[rr.pk,'approved'])).status_code,403)

 def test_non_owner_cannot_delete_equipment(self):
  self.client.login(username='other',password='StrongPass123!')
  response=self.client.get(reverse('equipment_delete',args=[self.eq.pk]))
  self.assertEqual(response.status_code,403)

 def test_owner_can_access_delete_equipment(self):
  self.client.login(username='owner',password='StrongPass123!')
  response=self.client.get(reverse('equipment_delete',args=[self.eq.pk]))
